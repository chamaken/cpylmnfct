...
class Filter(object):
    def __init__(self):			self._filter = conntrack.filter_create()
    ...
    @staticmethod
    def detach(fd):			conntrack.filter_detach(fd)
    def __enter__(self):		return self
    def __exit__(self, t, v, tb):
        self.destroy()
        return False
...
class FilterDump(object):
    def __init__(self):			self._filter_dump = conntrack.filter_dump_create()

    def destroy(self):
        conntrack.filter_dump_destroy(self._filter_dump)
        del self._filter_dump

    def __del__(self):		        hasattr(self, "_filter_dump") and self.destroy()
    def set_attr(self, a, v):		conntrack.filter_dump_set_attr(self._filter_dump, a, v)
    def set_attr_u8(self, a, v):	conntrack.filter_dump_set_attr_u8(self._filter_dump, a, v)
    def __enter__(self):		return self
    def __exit__(self, t, v, tb):
        self.destroy()
        return False
...
class Labelmap(object):
    def __init__(self, mapfile):	self._labelmap = conntrack.labelmap_new(mapfile)
...
class Bitmask(object):
    ...
    def clone(self):			return Bitmask(0, conntrack.bitmask_clone(self._bitmask))
