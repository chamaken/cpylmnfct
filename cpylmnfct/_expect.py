...
from . import _cproto
from ._libnetfilter_conntrackh import ATTR_EXP_MAX
...
def expect_get_attr_u8(exp, attr_type):
    ctypes.set_errno(0)
    ret = _cproto.c_nfexp_get_attr_u8(exp, attr_type)
...
def expect_get_attr_u16(exp, attr_type):
    ctypes.set_errno(0)
    ret = _cproto.c_nfexp_get_attr_u16(exp, attr_type)
    if ret == 0: _cproto.c_raise_if_errno()
    return ret
...
def expect_snprintf(size, exp, msg_type, out_flag, flags):
    ...
    ret = _cproto.c_nfexp_snprintf(ctypes.byref(c_buf), size, exp, msg_type, out_type, flags)
    if ret == -1: raise _cproto.os_error()
    return str(c_buf)

# expect_nlmsg_build = c_nfexp_nlmsg_build
def expect_nlmsg_build(nlh, exp):
    ret = _cproto.c_nfexp_nlmsg_build(nlh, exp)
    if ret == -1: raise _cproto.os_error()

# expect_nlmsg_parse = c_nfexp_nlmsg_parse
def expect_nlmsg_parse(nlh, ct):
    ret = _cproto.c_nfexp_nlmsg_parse(nlh, exp)
    if ret == -1: raise _cproto.os_error()
    return ret
