...
def main():
    ...
    with mnl.Socket(netlink.NETLINK_NETFILTER) as nl:
        skfilter = nfct.Filter()
        filter_mark = nfct.FilterMark(value=1, mask=3)
        # skfilter.set_logic(nfct.NFCT_FILTER_MARK, nfct.NFCT_FILTER_LOGIC_NEGATIVE)
        skfilter.add_attr(nfct.NFCT_FILTER_MARK, filter_mark);
        skfilter.add_attr_u32(nfct.NFCT_FILTER_L4PROTO, socket.IPPROTO_TCP);
        skfilter.attach(nl.get_fd())
