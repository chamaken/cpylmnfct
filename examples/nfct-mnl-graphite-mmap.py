        ...
        for efd, event in events:
            if efd == fd:
                ...
                if event == select.POLLERR:
                    s = socket.fromfd(nl.get_fd(), socket.AF_NETLINK, socket.SOCK_RAW)
...
def main():
    ...
    nl.set_ringopt(mnl.MNL_RING_RX, mnl.MNL_SOCKET_BUFFER_SIZE, 64, mnl.MNL_SOCKET_BUFFER_SIZE / 4, 4 * 64)
    nl.map_ring()
    ...
    sock.setsockopt(socket.SOL_SOCKET, 33, 1 << 22) # SO_RCVBUFFORCE
    on = struct.pack("i", 1)
    ...
    cpbuf = bytearray(mnl.MNL_SOCKET_BUFFER_SIZE)
    while True:
        ...
        if frame.status == netlink.NL_MMAP_STATUS_VALID:
            ...
            rsize = nl.recv_into(cpbuf) # XXX: no exception check
            buf = cpbuf[:rsize]
        else:
            mnl_socket_poll(nl) # XXX: no exception check
            continue
